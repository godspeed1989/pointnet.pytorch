...
class Modelnet40_PCL_Dataset(data.Dataset):
    def __init__(self, data_dir, npoints = 2048, train = True):
        ...
        if self.train:
            ...
            self.classes = np.unique(self.test_labels)

    def __getitem__(self, index):
        if self.train:
            points, label = self.train_data[index], (int)(self.train_labels[index])
        else:
            points, label = self.test_data[index], (int)(self.test_labels[index])
        return points, label
...
if __name__ == '__main__':
    print('test')
    d = Modelnet40_PCL_Dataset(data_dir = 'modelnet40_ply_hdf5_2048', train=True)
    print(len(d))
    points, label = d[0]
    print(points.shape)
    print(points.dtype)
    ...
    d = Modelnet40_PCL_Dataset(data_dir = 'modelnet40_ply_hdf5_2048', train=False)
    print(len(d))
    points, label = d[0]
    print(points.shape)
    print(points.dtype)
